...
void AI_Crazypants(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->mind2==0)		// when mind2=0, singlemindedly drift towards Bouapha
	{
		...
		{
			me->mind2=1;	// bumped a wall, so randomly maneuver
...
void AI_Yeti2(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		if(me->seq==ANIM_ATTACK && me->frm==7 && me->frmTimer<32)
		{
			...
			g=AddBaby(x,y,FIXAMT*10,MONS_SNOWBALL+Random(2),me);	// randomly pick which type of snowball, normal or reverse
...
void AI_Spikey(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_ATTACK && me->frm>=2 && me->frm<=6)
		{
			x=me->x+Cosine(me->facing*32)*16;
			y=me->y+Sine(me->facing*32)*16;
	...
	if(me->mind==0)		// when mind=0, singlemindedly drift towards Bouapha
	{
		...
		if(goodguy)
		{
			if(RangeToTarget(me,goodguy)<(512*FIXAMT) && Random(8)==0)
			{
				// get him!
...
{
	Guy *g;
	int summonTab[]={MONS_PUMPKIN,MONS_PUMPKIN,MONS_BOOMKIN,MONS_SQUASH,MONS_BEETLE,MONS_FROG2};
		...
		if(me->seq==ANIM_A1 && me->frm==7 && me->reload==0)
		{
			switch(me->aiType){
				...
				case MONS_PATCH4:
					...
					break;
				default:
					g=AddBaby(me->x,me->y+FIXAMT*2,0,MGL_randoml(NUM_MONSTERS-2)+2,me);
					break;
			}
			if(g)
			{
...
void AI_Dozer(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->hp<=700 && me->mind<2)	// lose arms at 400hp
	{
		me->seq=ANIM_A2;
		me->frm=0;
		me->frmTimer=0;
	...
	if(me->hp<=400 && me->mind<3)	// lose grill at 200hp
	{
		me->seq=ANIM_A3;
		me->frm=0;
		me->frmTimer=0;
...
void AI_Werewolf(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->aiType==MONS_WOLF2 && me->mind3==0 && me->mind2==0 && Random(128)==0)
	{
		// howl
